import Grafo from "../models/Grafo/Grafo.mjs";
...
document.getElementById("calcularRuta").addEventListener("click", function() {
    ...
    if (!inicio || !destino || !grafo.obtenerIndiceVertice(inicio) || !grafo.obtenerIndiceVertice(destino)) {
        alert("Ingrese vértices de inicio y destino válidos");
        return;
    }

    const resultado = rutaMasCorta(grafo, inicio, destino);
    const contenedorResultado = document.getElementById("resultadoRuta");
    contenedorResultado.innerHTML = `Distancia mínima: ${resultado.distancia}<br>Ruta: ${resultado.rutaIndices.join(" => ")}`;
});